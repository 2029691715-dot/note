> 原文链接：https://docs.python.org/3/howto/urllib2.html
...
the [`urllib.request`](../library/urllib.request.html#module-urllib.request) docs, but is supplementary to them.
...
location, you can do so via the [`shutil.copyfileobj()`](../library/shutil.html#shutil.copyfileobj) and
[`tempfile.NamedTemporaryFile()`](../library/tempfile.html#tempfile.NamedTemporaryFile) functions:
...
argument. The encoding is done using a function from the [`urllib.parse`](../library/urllib.parse.html#module-urllib.parse)
library.
...
Some websites [\[1\]](#id7) dislike being browsed by programs, or send different versions
to different browsers [\[2\]](#id8). By default urllib identifies itself as
`Python-urllib/x.y` (where `x` and `y` are the major and minor version
numbers of the Python release,
e.g. `Python-urllib/2.5`), which may confuse the site, or just plain
not work. The way a browser identifies itself is through the
`User-Agent` header [\[3\]](#id9). When you create a Request object you can
...
*urlopen* raises [`URLError`](../library/urllib.error.html#urllib.error.URLError) when it cannot handle a response (though as
usual with Python APIs, built-in exceptions such as [`ValueError`](../library/exceptions.html#ValueError),
[`TypeError`](../library/exceptions.html#TypeError) etc. may also be raised).

[`HTTPError`](../library/urllib.error.html#urllib.error.HTTPError) is the subclass of [`URLError`](../library/urllib.error.html#urllib.error.URLError) raised in the specific case of
HTTP URLs.

The exception classes are exported from the [`urllib.error`](../library/urllib.error.html#module-urllib.error) module.
...
urlopen will raise an [`HTTPError`](../library/urllib.error.html#urllib.error.HTTPError). Typical errors include ‘404’ (page not
...
The [`HTTPError`](../library/urllib.error.html#urllib.error.HTTPError) instance raised will have an integer ‘code’ attribute, which
...
[`http.server.BaseHTTPRequestHandler.responses`](../library/http.server.html#http.server.BaseHTTPRequestHandler.responses) is a useful dictionary of
...
*and* an error page. You can use the [`HTTPError`](../library/urllib.error.html#urllib.error.HTTPError) instance as a response on the
...
So if you want to be prepared for `HTTPError`*or* [`URLError`](../library/urllib.error.html#urllib.error.URLError) there are two
...
will *also* catch an [`HTTPError`](../library/urllib.error.html#urllib.error.HTTPError).
...
The response returned by urlopen (or the [`HTTPError`](../library/urllib.error.html#urllib.error.HTTPError) instance) has two
...
fetched, particularly the headers sent by the server. It is currently an[`http.client.HTTPMessage`](../library/http.client.html#http.client.HTTPMessage) instance.
...
confusingly named [`urllib.request.OpenerDirector`](../library/urllib.request.html#urllib.request.OpenerDirector)). Normally we have been using
...
when it may not be helpful [\[5\]](#id11). One way to do this is to setup our own
`ProxyHandler`, with no proxies defined. This is done using similar steps to
setting up a [Basic Authentication](#basic-authentication) handler:
...
shown in the recipe [\[6\]](#id12).

Note

`HTTP_PROXY` will be ignored if a variable `REQUEST_METHOD` is set; see
the documentation on [`getproxies()`](../library/urllib.request.html#urllib.request.getproxies).

## Sockets and Layers[¶](#sockets-and-layers)

The Python support for fetching resources from the web is layered.  urllib uses
the [`http.client`](../library/http.client.html#module-http.client) library, which in turn uses the socket library.
